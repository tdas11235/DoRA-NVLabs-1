...
def main(
    ...
    if args.adapter == "LoRA" or args.adapter == "DoRA" or args.adapter == "FrobDoRA" or args.adapter == "OLoRA":
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=["boolq", "piqa", "social_i_qa", "hellaswag", "winogrande", "ARC-Challenge", "ARC-Easy", "openbookqa"],
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', "LLaMA-13B",'LLaMA2-7B','LLaMA3-8B', 'llama-68m', 'phi-3-4k-instruct'], required=True)
    parser.add_argument('--adapter', choices=['None', 'LoRA', 'AdapterP', 'AdapterH', 'Parallel', 'DoRA', 'FrobDoRA', 'OLoRA'],
                        required=True)
