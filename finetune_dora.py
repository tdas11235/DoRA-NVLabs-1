...
def get_MD(module):
        # Magnitude
        M = module.weight_m_wdecomp.weight.detach().clone()
...
class DoraTrackingCallback(TrainerCallback):
...
def train(
    ...
    for name, module in model.named_modules():
        if hasattr(module, "weight_m_wdecomp") and "qkv_proj" in name:
            dora_layers.append((name, module))

    print(f"Found {len(dora_layers)} DoRA layers")
    ...
    for name, module in dora_layers:
    ...
    tracking_callback = DoraTrackingCallback(dora_layers, initial_MD, TARGET_STEPS)
