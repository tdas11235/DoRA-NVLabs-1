...
def get_MD(module):
        # Direction
        if hasattr(module, "lora_A"):  # full DoRA
            V = module.weight + (module.lora_B.weight @ module.lora_A.weight) * module.scaling
        else:  # Wdecompose case
            V = module.weight

        # Magnitude (row-wise norm)
        M = torch.linalg.norm(V, dim=1, keepdim=True)

        #norm = torch.linalg.norm(V, dim=1, keepdim=True) + 1e-8
        #D = V / norm
...
class LoraTrackingCallback(TrainerCallback):
    def __init__(self, lora_layers, initial_MD, target_steps):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        """
        if state.global_step % 50 != 0:  # reduce overhead
            return
        """
        ...
        for name, module in self.lora_layers:
            ...
            # ΔM
            delta_M = (M - M0).abs().mean().item()

            # ΔD via cosine similarity
            cos_sim = torch.sum(V * V0, dim=1) / (
                torch.linalg.norm(V, dim=1) * torch.linalg.norm(V0, dim=1) + 1e-8
            )
            delta_D = (1 - cos_sim).mean().item()

            step_data.append((delta_M, delta_D))
...
def train(
    ...
    for name, module in model.named_modules():
        if hasattr(module, "lora_A") and hasattr(module, "lora_B") and "qkv_proj" in name:
    ...
    for name, module in model.named_modules():
        if hasattr(module, "weight_m_wdecomp") and "q_proj" in name:
            dora_layers.append((name, module))

    print(f"Total Q DoRA layers: {len(dora_layers)}")
    """

    lora_layers = lora_layers[::2][:6]
    ...
    for name, module in lora_layers:
        M, V = get_MD(module)   # use V instead of D
        initial_MD[name] = {
            "M0": M,
            "V0": V
        }
    ...
    tracking_callback = LoraTrackingCallback(lora_layers, initial_MD, TARGET_STEPS)
    ...
    with open(os.path.join("/kaggle/working", "lora_logs.json"), "w") as f:
