...
@dataclass
class FrobDoraConfig(PeftConfig):
    """
    ...
    frobdora_simple: bool = field(
        default=True, metadata={"help": "Whether to apply simple frobdora ver to save up GPU memory"}
    )
    ...
    def __post_init__(self):
        self.peft_type = PeftType.FROBDORA


class FrobDoraModel(torch.nn.Module):
    """
    ...
    **Attributes**:
    ...
    def _find_and_replace(self):
        ...
        kwargs = {
            ...
            "frobdora_simple": self.peft_config.frobdora_simple
        }
    ...
    def _replace_module(self, parent_module, child_name, new_module, old_module):
        ...
        # 
        with torch.no_grad():
            frob_norm = torch.linalg.norm(new_module.weight.detach())
            new_module.m_scalar.data.copy_(frob_norm.unsqueeze(0))
        ...
        for name, module in new_module.named_modules():
            if "lora_" in name or "m_scalar" in name:
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = "none") -> None:
    for n, p in model.named_parameters():
        if "lora_" not in n and "m_scalar" not in n:
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.m_scalar = nn.Parameter(torch.ones(1)) # single scalar now

        self.fan_in_fan_out = fan_in_fan_out
        self.Wdecompose = Wdecompose # whether to tune only the magnitude component of Wdecompose or not
        self.frobdora_simple = frobdora_simple # whether to use dora simple to save up GPU memory
    ...
    def train(self, mode: bool = True):
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.Wdecompose:
                fro_norm = torch.linalg.norm(self.weight) + 1e-6
                norm_scale = self.m_scalar / fro_norm
                ...
                if self.r > 0:
                    ...
                    fro_norm = torch.linalg.norm(new_weight_v) + 1e-6
                    norm_scale = self.m_scalar / fro_norm
                    weight = norm_scale * new_weight_v
                    self.weight.data.copy_(weight.detach())
            self.merged = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.frobdora_simple:
                fro_norm = torch.linalg.norm(new_weight_v.detach()) + 1e-6
                norm_scale = self.m_scalar / fro_norm
            else:
                fro_norm = torch.linalg.norm(new_weight_v) + 1e-6
                norm_scale = self.m_scalar / fro_norm
